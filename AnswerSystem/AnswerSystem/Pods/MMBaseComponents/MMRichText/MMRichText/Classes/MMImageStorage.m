...
- (void)setOwnerView:(UIView *)ownerView
{
    ...
    if ([_imageURL isKindOfClass:[NSURL class]]
        && ![[MMImageCache cache] imageIsCacheForURL:_imageURL.absoluteString]) {
        
        [[MMImageCache cache]saveAsyncImageFromURL:_imageURL.absoluteString thumbImageSize:self.size completion:^(BOOL isCache) {
...
- (void)drawStorageWithRect:(CGRect)rect
{
    ...
    if (_image) {
        ...
        // 图片数据
        [[MMImageCache cache] imageForURL:_imageURL.absoluteString needThumImage:NO found:^(UIImage *loaceImage) {
            image = loaceImage;
            if (_cacheImageOnMemory) {
                _image = image;
            }
...
- (CGRect)rectFitOriginSize:(CGSize)size byRect:(CGRect)byRect{
    ...
    if (_imageAlignment == MMImageAlignmentFill) {
        return byRect;
    }
