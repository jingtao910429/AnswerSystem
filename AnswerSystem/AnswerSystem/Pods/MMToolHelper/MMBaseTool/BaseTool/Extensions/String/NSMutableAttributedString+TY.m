 ...
 *  @param color 文本颜色
 */
- (void)addAttributeBackGroundColor:(UIColor*)color {
    [self addAttributeBackGroundColor:color range:NSMakeRange(0, [self length])];
}
...
- (void)addAttributeBackGroundColor:(UIColor*)color range:(NSRange)range {
    ...
    if (color.CGColor)
    {
        if (@available(iOS 10.0, *)) {
            [self removeAttribute:(NSString *)kCTBackgroundColorAttributeName range:range];
